...
type
  CreateOptionsPlannedWorkSelect = record
    FormCaption: string;
  end;

type
  TPlannedWorkSelectForm = class(TForm)
    WorkQ: TADOQuery;
    WorkDS: TDataSource;
...
var
  PlannedWorkSelectForm: TPlannedWorkSelectForm;
function ShowPlannedWorkSelect(Options: CreateOptionsPlannedWorkSelect): Integer;
...
procedure TPlannedWorkSelectForm.RefreshData;
...
begin
  try
    ...
    OpenQ(StageQ, 'exec StagesShow ' + VariantAsString(BudgetGUID), False);
  except
  end;
end;
...
begin
  Result := -1;
  try
    Application.CreateForm(TPlannedWorkSelectForm, PlannedWorkSelectForm);
    PlannedWorkSelectForm.Caption := Options.FormCaption;
    PlannedWorkSelectForm.Show;
    Result := 1;
  except
    PlannedWorkSelectForm.Free;
    PlannedWorkSelectForm := nil;
  end;
end;

procedure TPlannedWorkSelectForm.btnAddRecordClick(Sender: TObject);
begin
  try
    ...
    MainForm.Query.Parameters.ParamByName('FormName').Value := 'PlannedWork';
...
procedure TPlannedWorkSelectForm.btnColumnAutoWidthClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.ColumnAutoWidth :=
    not(cxGrid1DBTableView1.optionsView.ColumnAutoWidth);
end;

procedure TPlannedWorkSelectForm.btnDecreaseFontClick(Sender: TObject);
begin
...
procedure TPlannedWorkSelectForm.btnIncreaseFontClick(Sender: TObject);
begin
  cxGrid1DBTableView1.BeginUpdate;
  with cxGrid1DBTableView1.Styles do
    begin
...
procedure TPlannedWorkSelectForm.btnWordWrapClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.CellAutoHeight :=
    not(cxGrid1DBTableView1.optionsView.CellAutoHeight);
end;

procedure TPlannedWorkSelectForm.ChangeFontSize(S: TcxGridTableViewStyles;
  SizeDifference: Integer = 0);
var
  i: Integer;
begin
...
procedure TPlannedWorkSelectForm.cxButton1Click(Sender: TObject);
begin
  messagedlg('Экспорт в Excel', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedWorkSelectForm.cxButton2Click(Sender: TObject);
begin
  messagedlg('Печать', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedWorkSelectForm.cxGrid1DBTableView1CustomDrawCell
...
procedure TPlannedWorkSelectForm.
...
procedure TPlannedWorkSelectForm.cxGridDBTableView1FocusedRecordChanged(Sender: TcxCustomGridTableView;
  APrevFocusedRecord, AFocusedRecord: TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
begin
  RefreshData;
  try
    WorkQ.Close;
    WorkQ.SQL.Text := 'exec plannedWorkShow :BudgetGUID';
...
procedure TPlannedWorkSelectForm.FormActivate(Sender: TObject);
begin
  MainForm.ActivateTab(MainForm.TabSet, TForm(Sender));
end;

procedure TPlannedWorkSelectForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TPlannedWorkSelectForm.FormShow(Sender: TObject);
begin
...
procedure TPlannedWorkSelectForm.WorkQAfterPost(DataSet: TDataSet);
begin
  RL.RecordControlID := DataSet.FieldByName('ID').AsInteger;
end;

procedure TPlannedWorkSelectForm.WorkQBeforePost(DataSet: TDataSet);
begin
  RL.ErrorState := False;
  RL.ErrorControl := true;
  WorkQ.FieldByName('PostGUID').Value := PostQ.FieldByName('GUID').AsVariant;
  WorkQ.FieldByName('DivisionGUID').Value := DepartmentQ.FieldByName('GUID')
    .AsVariant;
  WorkQ.FieldByName('BudgetGUID').AsVariant := GlobalBudgetGUID;
end;

procedure TPlannedWorkSelectForm.WorkQPostError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
