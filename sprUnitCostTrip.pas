...
uses
  ...
  ExtCtrls, dxStatusBar, cxDBLookupComboBox, Main, SQLRoutines1, dxPSGlbl, dxPSUtl, dxPSEngn, dxPrnPg, dxBkgnd, dxWrap,
...
type
  TsprCostTripForm = class(TGridVewTemplateFormSource)
    CityStartQuery: TADOQuery;
    CityStartDataset: TDataSource;
    CityEndQuery: TADOQuery;
    CityEndDataset: TDataSource;
    TypesTransportQuery: TADOQuery;
    TypesTransportDataset: TDataSource;
    ...
    cxGrid1DBTableView2CityGUIDStart: TcxGridDBColumn;
    cxGrid1DBTableView2CityGUIDFinish: TcxGridDBColumn;
    cxGrid1DBTableView2TypeTransportGUID: TcxGridDBColumn;
...
var
  sprCostTripForm: TsprCostTripForm;
...
begin
  ...
  if not(Assigned(sprCostTripForm)) then // Если еще не создана
    begin
      try
        Application.CreateForm(TsprCostTripForm, sprCostTripForm);
  ...
  if (FormCreatedOrExists = true) then
    ...
    begin
      ...
      OpenParamsQ(sprCostTripForm.Adoquery1,'exec CostsTripShow :BudgetGUID, :FormType', [GlobalBudgetGUID, #39 + 'Справочник' + #39]);

      sprCostTripForm.Adoquery4.Close;
      sprCostTripForm.Adoquery4.SQL.Text := 'exec SprCostsTripJournalShow :BudgetGUID';
      sprCostTripForm.Adoquery4.Parameters.ParamByName('BudgetGUID').Value := GlobalBudgetGUID;
      sprCostTripForm.Adoquery4.Open;

      OpenQ(sprCostTripForm.ADOQuery2, 'exec ApplicationTablesShow');


       OpenParamsQ(sprCostTripForm.CityStartQuery,'exec CitiesShow :BudgetGUID, :FormType', [GlobalBudgetGUID, #39 + 'Справочник' + #39]);



       OpenParamsQ(sprCostTripForm.CityEndQuery,'exec CitiesShow :BudgetGUID, :FormType', [GlobalBudgetGUID, #39 + 'Справочник' + #39]);


        OpenParamsQ(sprCostTripForm.TypesTransportQuery,'exec TypesTransportShow :BudgetGUID, :FormType', [GlobalBudgetGUID, #39 + 'Справочник' + #39]);
      sprCostTripForm.Visible := true;
      sprCostTripForm.BringToFront;
    end;
end;

procedure TsprCostTripForm.ADOQuery1AfterPost(DataSet: TDataSet);
begin
  inherited;
      sprCostTripForm.Adoquery4.Close;
      sprCostTripForm.Adoquery4.SQL.Text := 'exec SprCostsTripJournalShow :BudgetGUID';
...
procedure TsprCostTripForm.ADOQuery1BeforePost(DataSet: TDataSet);
var exb:word;
begin
  inherited;
...
procedure TsprCostTripForm.cxGrid1DBTableView1CustomDrawCell(Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
