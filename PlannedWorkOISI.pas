...
uses
  ...
  Common, cxDBExtLookupComboBox, Main;


type
  TPlannedWorkOISIForm = class(TForm)
    WorkQ: TADOQuery;
    WorkDS: TDataSource;
    ...
    cxGridDBTableView1DayCount: TcxGridDBColumn;
    cxGridDBTableView1EmployeeCount: TcxGridDBColumn;
    cxGridDBTableView1FieldContentmentTotal: TcxGridDBColumn;
...
var
  PlannedWorkOISIForm: TPlannedWorkOISIForm;
function ShowPlannedWorkOISI(Options: CreateOptionsGlobal): Integer;
...
procedure TPlannedWorkOISIForm.RefreshData;
...
begin
  try
    ...
    OpenParamsQ(FieldContentmentQ, 'exec FieldContentmentsShow :BudgetGUID, :FormType', [BudgetGUIDForSpr, 'Форма ввода']);
  except
  end;
end;
...
begin
  Result := -1;
  try
    Application.CreateForm(TPlannedWorkOISIForm, PlannedWorkOISIForm);
    PlannedWorkOISIForm.Caption := Options.FormCaption;
    PlannedWorkOISIForm.BudgetGUID := Options.BudgetGUID;
    PlannedWorkOISIForm.BudgetGUIDForSpr := Options.BudgetGUIDForSpr;
    PlannedWorkOISIForm.Show;
    Result := 1;
  except
    PlannedWorkOISIForm.Free;
    PlannedWorkOISIForm := nil;
  end;
end;

procedure TPlannedWorkOISIForm.btnColumnAutoWidthClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.ColumnAutoWidth :=
    not(cxGrid1DBTableView1.optionsView.ColumnAutoWidth);
end;

procedure TPlannedWorkOISIForm.btnDecreaseFontClick(Sender: TObject);
begin
...
procedure TPlannedWorkOISIForm.btnIncreaseFontClick(Sender: TObject);
begin
  cxGrid1DBTableView1.BeginUpdate;
  with cxGrid1DBTableView1.Styles do
    begin
...
procedure TPlannedWorkOISIForm.btnWordWrapClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.CellAutoHeight :=
    not(cxGrid1DBTableView1.optionsView.CellAutoHeight);
end;

procedure TPlannedWorkOISIForm.BudgetAllClick(Sender: TObject);
begin
  ShowPlanData(TcxButton(Sender));
end;

procedure TPlannedWorkOISIForm.BudgetNIOClick(Sender: TObject);
begin
  ShowPlanData(TcxButton(Sender));
end;

procedure TPlannedWorkOISIForm.BudgetOANClick(Sender: TObject);
begin
  ShowPlanData(TcxButton(Sender));
end;

procedure TPlannedWorkOISIForm.BudgetOISIClick(Sender: TObject);
begin
  ShowPlanData(TcxButton(Sender));
end;

procedure TPlannedWorkOISIForm.BudgetProClick(Sender: TObject);
begin
  ShowPlanData(TcxButton(Sender));
end;

procedure TPlannedWorkOISIForm.CalcTypeBoxPropertiesCloseUp(Sender: TObject);
begin
  cxGrid1.SetFocus;
end;

procedure TPlannedWorkOISIForm.ChangeFontSize(S: TcxGridTableViewStyles;
  SizeDifference: Integer = 0);
var
  i: Integer;
begin
...
procedure TPlannedWorkOISIForm.cxButton2Click(Sender: TObject);
begin
  dxComponentPrinter1Link1.Preview(true);
end;

procedure TPlannedWorkOISIForm.cxButton3Click(Sender: TObject);
begin
  messagedlg('Печать', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedWorkOISIForm.cxGrid1DBTableView1CustomDrawCell
...
procedure TPlannedWorkOISIForm.cxGrid1DBTableView1Editing(Sender: TcxCustomGridTableView; AItem: TcxCustomGridTableItem;
  var AAllow: Boolean);
...
procedure TPlannedWorkOISIForm.cxGrid1DBTableView1EditKeyDown(Sender: TcxCustomGridTableView;
...
procedure TPlannedWorkOISIForm.cxGrid1DBTableView1FocusedRecordChanged(Sender: TcxCustomGridTableView;
...
procedure TPlannedWorkOISIForm.cxGrid1DBTableView1InitEdit(Sender: TcxCustomGridTableView; AItem: TcxCustomGridTableItem;
  AEdit: TcxCustomEdit);
...
procedure TPlannedWorkOISIForm.cxGrid1DBTableView1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = 46 then
    begin
      ...
      if MessageDlg('Удалить?', mtConfirmation,[mbYes, mbNo], 0) = mrYes then
        begin
          try
            ...
            MainForm.Query.Parameters.ParamByName('Plan').Value := 'PlannedWorkOISI';
            ...
            RefreshPlan(0);
            MainForm.PlanGetRecordCount(MainForm.Query, 'PlannedWorkOISI', TForm(PlannedWorkOISIForm));
...
procedure TPlannedWorkOISIForm.cxGridDBTableView1EditKeyDown(Sender: TcxCustomGridTableView;
  AItem: TcxCustomGridTableItem; AEdit: TcxCustomEdit; var Key: Word; Shift: TShiftState);
begin
...
procedure TPlannedWorkOISIForm.cxGridDBTableView1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Key = 13) and (cxGridDBTableView1.Controller.FocusedColumnIndex = cxGridDBTableView1.VisibleColumnCount-1) then
    begin
      if (MonthQ.State = dsEdit) or (MonthQ.State = dsInsert) then
...
procedure TPlannedWorkOISIForm.FormClose(Sender: TObject;
...
procedure TPlannedWorkOISIForm.FormShow(Sender: TObject);
...
procedure TPlannedWorkOISIForm.FormAccessControl;
begin
  try
...
procedure TPlannedWorkOISIForm.FormActivate(Sender: TObject);
begin
  MainForm.ActivateTab(MainForm.TabSet, TForm(Sender));
end;

procedure TPlannedWorkOISIForm.ImageButton1Click(Sender: TObject);
begin
  MainForm.UploadToExcel(cxGrid1, Caption+'_'+ButtonDownName);
    MainForm.UploadToExcel(cxGrid2, Caption+'_'+ButtonDownName);
end;

procedure TPlannedWorkOISIForm.ImageButton3Click(Sender: TObject);
begin
...
procedure TPlannedWorkOISIForm.MonthQAfterPost(DataSet: TDataSet);
begin
  PlannedUpdateSumCounts;
end;

procedure TPlannedWorkOISIForm.PlannedUpdateSumCounts;
begin
      try
        MainForm.Query.Close;
...
procedure TPlannedWorkOISIForm.RefreshMonths(NewItem: Boolean);
begin
  try
      if not NewItem then
      begin
    MonthQ.Close;
    MonthQ.SQL.Text := 'exec PlannedWorkOISIMonthsShow :BudgetGUID, :PlannedID';
...
procedure TPlannedWorkOISIForm.WorkQAfterOpen(DataSet: TDataSet);
begin
  MainForm.PlanGetRecordCount(MainForm.Query, 'PlannedWorkOISI', PlannedWorkOISIForm);
end;

procedure TPlannedWorkOISIForm.WorkQAfterPost(DataSet: TDataSet);
begin
  MainForm.PlanGetRecordCount(MainForm.Query, 'PlannedWorkOISI', TForm(PlannedWorkOISIForm));
  RL.RecordControlID := DataSet.FieldByName('ID').AsInteger;
      try
        ...
        MainForm.Query.Close;
        MainForm.Query.SQL.Text := 'exec PlannedWorkOISIFillAndCleanMonths :BudgetGUID';
        MainForm.Query.Parameters.ParamByName('BudgetGUID').Value := GlobalBudgetGUID;
        MainForm.Query.ExecSQL;

        MainForm.Query.Close;
        MainForm.Query.SQL.Text := 'exec PlannedWorkOISIMonthsCalculate :BudgetGUID, :PlannedID';
        MainForm.Query.Parameters.ParamByName('BudgetGUID').Value := GlobalBudgetGUID;
        MainForm.Query.Parameters.ParamByName('PlannedID').Value := WorkQ.FieldByName('ID').AsVariant;
        MainForm.Query.ExecSQL;

        WorkQ.Close;
        WorkQ.SQL.Text := 'exec plannedWorkOISIShow :BudgetGUID, :CalcType';
...
procedure TPlannedWorkOISIForm.WorkQBeforeInsert(DataSet: TDataSet);
begin
  QueryState := 0;
end;

procedure TPlannedWorkOISIForm.WorkQBeforePost(DataSet: TDataSet);
begin
  RL.ErrorState := False;
  ...
  if QueryState = 0 then
    begin
      ...
      //WorkQ.FieldByName('DayCount').Value := 0;
      //WorkQ.FieldByName('HourCount').Value := 0;
    end;
...
procedure TPlannedWorkOISIForm.WorkQPostError(DataSet: TDataSet;
...
procedure TPlannedWorkOISIForm.RefreshPlan(WithSaveID: Integer);
...
procedure TPlannedWorkOISIForm.ShowPlanData(Button: TcxButton);
begin
