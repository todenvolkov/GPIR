...
type
  TPlannedTransportSelectForm = class(TForm)
    WorkQ: TADOQuery;
    WorkDS: TDataSource;
...
var
  PlannedTransportSelectForm: TPlannedTransportSelectForm;
  function ShowPlannedTransportSelect(Options: CreateOptionsGlobal): Integer;
...
procedure TPlannedTransportSelectForm.RefreshData;
...
begin
  try
    ...
    OpenQ(BudgetArticleQ, 'exec BudgetArticlesShow ' + VariantAsString(BudgetGUID), False);
...
begin
  Result := -1;
  try
    Application.CreateForm(TPlannedTransportSelectForm, PlannedTransportSelectForm);
    PlannedTransportSelectForm.Caption := Options.FormCaption;
    PlannedTransportSelectForm.Show;
    Result := 1;
  except
    PlannedTransportSelectForm.Free;
    PlannedTransportSelectForm := nil;
  end;
end;

procedure TPlannedTransportSelectForm.btnAddRecordClick(Sender: TObject);
begin
  try
    ...
    MainForm.Query.Parameters.ParamByName('FormName').Value := 'PlannedTransport';
...
procedure TPlannedTransportSelectForm.btnColumnAutoWidthClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.ColumnAutoWidth :=
    not(cxGrid1DBTableView1.optionsView.ColumnAutoWidth);
end;

procedure TPlannedTransportSelectForm.btnDecreaseFontClick(Sender: TObject);
begin
...
procedure TPlannedTransportSelectForm.btnIncreaseFontClick(Sender: TObject);
begin
  cxGrid1DBTableView1.BeginUpdate;
  with cxGrid1DBTableView1.Styles do
    begin
...
procedure TPlannedTransportSelectForm.btnWordWrapClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.CellAutoHeight :=
    not(cxGrid1DBTableView1.optionsView.CellAutoHeight);
end;

procedure TPlannedTransportSelectForm.ChangeFontSize(S: TcxGridTableViewStyles;
  SizeDifference: Integer = 0);
var
  i: Integer;
begin
...
procedure TPlannedTransportSelectForm.cxButton1Click(Sender: TObject);
begin
  messagedlg('Экспорт в Excel', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedTransportSelectForm.cxButton2Click(Sender: TObject);
begin
  messagedlg('Печать', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedTransportSelectForm.cxGrid1DBTableView1CustomDrawCell
...
procedure TPlannedTransportSelectForm.cxGridDBTableView1FocusedRecordChanged(Sender: TcxCustomGridTableView;
  APrevFocusedRecord, AFocusedRecord: TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
begin
  RefreshData;
  try
    WorkQ.Close;
    WorkQ.SQL.Text := 'exec plannedTransportShow :BudgetGUID';
...
procedure TPlannedTransportSelectForm.FormActivate(Sender: TObject);
begin
  MainForm.ActivateTab(MainForm.TabSet, TForm(Sender));
end;

procedure TPlannedTransportSelectForm.FormClose(Sender: TObject;
...
procedure TPlannedTransportSelectForm.FormShow(Sender: TObject);
begin
...
procedure TPlannedTransportSelectForm.WorkQAfterPost(DataSet: TDataSet);
begin
...
procedure TPlannedTransportSelectForm.WorkQBeforePost(DataSet: TDataSet);
begin
  RL.ErrorState := False;
...
procedure TPlannedTransportSelectForm.WorkQPostError(DataSet: TDataSet;
