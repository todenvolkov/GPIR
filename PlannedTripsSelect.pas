...
uses
  ...
  cxButtonEdit, cxDBLookupComboBox, Main, TreeUnit;
...
type
  TPlannedTripsSelectForm = class(TForm)
    WorkQ: TADOQuery;
    WorkDS: TDataSource;
    ...
    procedure cxGrid1DBTableView1CustomDrawCell(Sender: TcxCustomGridTableView;
      ...
      var ADone: Boolean);
    procedure cxGrid1DBTableView1EmployeeGUIDPropertiesEditValueChanged
      (Sender: TObject);
...
var
  PlannedTripsSelectForm: TPlannedTripsSelectForm;
  function ShowPlannedTripsSelect(Options: CreateOptionsGlobal): Integer;
...
procedure TPlannedTripsSelectForm.RefreshData;
...
begin
  try
    ...
    OpenQ(CostResidingQ, 'exec CostResidingShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(CostTripQ, 'exec CostsTripShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(DailyAllowanceQ, 'exec DailyAllowancesShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(CityStartQ, 'exec CitiesShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(CityFinishQ, 'exec CitiesShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(TypeWorkQ, 'exec TypesWorkShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(PlaceResidenceQ, 'exec TypesTransportShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(TypeTransportQ, 'exec TypesTransportShow ' + VariantAsString(BudgetGUID), False);
    OpenQ(EmployeeQ, 'exec EmployeesShow ' + VariantAsString(BudgetGUID), False);
...
begin
  Result := -1;
  try
    Application.CreateForm(TPlannedTripsSelectForm, PlannedTripsSelectForm);
    PlannedTripsSelectForm.Caption := Options.FormCaption;
    PlannedTripsSelectForm.Show;
    Result := 1;
  except
    PlannedTripsSelectForm.Free;
    PlannedTripsSelectForm := nil;
  end;
end;

procedure TPlannedTripsSelectForm.btnAddRecordClick(Sender: TObject);
begin
  try
    ...
    MainForm.Query.Parameters.ParamByName('FormName').Value := 'PlannedTrips';
...
procedure TPlannedTripsSelectForm.btnColumnAutoWidthClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.ColumnAutoWidth :=
    not(cxGrid1DBTableView1.optionsView.ColumnAutoWidth);
end;

procedure TPlannedTripsSelectForm.btnDecreaseFontClick(Sender: TObject);
begin
...
procedure TPlannedTripsSelectForm.btnIncreaseFontClick(Sender: TObject);
begin
cxGrid1DBTableView1.BeginUpdate;
  with cxGrid1DBTableView1.Styles do
    begin
...
procedure TPlannedTripsSelectForm.btnWordWrapClick(Sender: TObject);
begin
  cxGrid1DBTableView1.optionsView.CellAutoHeight :=
    not(cxGrid1DBTableView1.optionsView.CellAutoHeight);
end;

procedure TPlannedTripsSelectForm.ChangeFontSize(S: TcxGridTableViewStyles;
  SizeDifference: Integer = 0);
var
  i: Integer;
begin
...
procedure TPlannedTripsSelectForm.cxButton1Click(Sender: TObject);
begin
  messagedlg('Экспорт в Excel', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedTripsSelectForm.cxButton2Click(Sender: TObject);
begin
  messagedlg('Печать', mtConfirmation, [mbYes], 0);
end;

procedure TPlannedTripsSelectForm.cxGrid1DBTableView1CustomDrawCell
...
procedure TPlannedTripsSelectForm.
  cxGrid1DBTableView1EmployeeGUIDPropertiesEditValueChanged(Sender: TObject);
begin
  PostQ.Locate('GUID', EmployeeQ.FieldByName('POSTGUID').AsVariant,
    [loCaseInsensitive]);
  DepartmentQ.Locate('GUID', EmployeeQ.FieldByName('DepartmentGUID').AsVariant,
    [loCaseInsensitive]);
end;

procedure TPlannedTripsSelectForm.cxGridDBTableView1FocusedRecordChanged(Sender: TcxCustomGridTableView;
  APrevFocusedRecord, AFocusedRecord: TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
begin
  RefreshData;
  try
    WorkQ.Close;
    WorkQ.SQL.Text := 'exec plannedTripsShow :BudgetGUID';
...
procedure TPlannedTripsSelectForm.FormActivate(Sender: TObject);
begin
  MainForm.ActivateTab(MainForm.TabSet, TForm(Sender));
end;

procedure TPlannedTripsSelectForm.FormClose(Sender: TObject;
...
procedure TPlannedTripsSelectForm.FormShow(Sender: TObject);
begin
...
procedure TPlannedTripsSelectForm.WorkQAfterPost(DataSet: TDataSet);
begin
  RL.RecordControlID := DataSet.FieldByName('ID').AsInteger;
  QueryPreCalc.Close;
  QueryPreCalc.SQL.Text := 'exec PlannedTripsPreCalc :ID, :BudgetGUID';
...
procedure TPlannedTripsSelectForm.WorkQBeforePost(DataSet: TDataSet);
begin
  RL.ErrorState := False;
...
procedure TPlannedTripsSelectForm.WorkQPostError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
